...
이 문제로 RDS에서 병목이 생긴 것으로 확인된다. 내가 구조를 잘 짰는 먼저 검토했다. `중간테이블을 만들었는데 나는 왜 cloth에 양방향 관계를 만든거지? 중간테이블에서 조회하면 되지 않나?` 문득 생각이 들었다. 바로 행동으로 옮겼다. 양방향 관계를 제거하고 옷을 조회할 때, `cltohWithAttributeRepository`에서 옷의 ID에 따른 속성테이블을 JOIN하여 갖고온다. 그 후, 서비스 단에서 `clothId`에 따른 속성들을 그룹화 해주고 DTO로 변환한다. 최종적으로 옷과 속성정보를 같이 조회할 수 있게된다. 따라서, 단방향 구조로 변환한 것이다.
